...
func (h *Handler) Execute(msg *tgbotapi.Message, params []string) error {
	...
	case "1":
		newState = true
	default:
		return ee.New(true, false, soundStateIncorrect)
	}
...
func (h *Handler) sendState(chatID int64, format string) error {
	...
	err := utils.SendBotMessage(h.Bot, chatID, resp, true, h.Conf.BotApi.DisableNotification)
	return ee.Wrap(err, true, false, cantSendBotMessage)
}
